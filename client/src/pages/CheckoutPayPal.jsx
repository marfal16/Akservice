...
const CheckoutPayPal = ({ totalAmount }) => {
  ...
  useEffect(() => {
    ...
    const renderPayPalButton = () => {
      window.paypal.Buttons({
        ...
        createOrder: (data, actions) => {
          return fetch("http://localhost:5000/api/paypal/create-order", {
            method: "POST",
            headers: {
    ...
    if (window.paypal) {
      ...
      script.src = "https://www.paypal.com/sdk/js?client-id=${process.env.VITE_PAYPAL_CLIENT_ID}&components=buttons&disable-funding=card,mybank,venmo&currency=EUR";
